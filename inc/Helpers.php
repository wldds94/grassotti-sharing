...
if ( ! function_exists('wlankPrintTitleInput') ) {
    ...
    { 
		return html_entity_decode( esc_html( $label ) . ( $description != '' ? ':<span class="text-light-weight small"> ' . esc_html( $description ) . '</span>' : '' ) );
    }
}
 ...
 * File Size
 */
if ( ! function_exists( 'prettyFileSize' ) ) {
	/**
	 * Return a pretty formatter of File Size
	 *
	 * Get the size without unit dimension
	 * 
	 * @param  float  $size
	 * @return string
	 */
	function prettyFileSize($size = false) {
